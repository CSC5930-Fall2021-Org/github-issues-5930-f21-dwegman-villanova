...
public class GitHubRestClient {
    ...
    public String getIssues(String token) {
        String responseBody = Unirest.get(TEST_AREA_URL).header("Authorization", "Bearer " + token).asString()
                .getBody();
        return responseBody;
    }
    
    public int getStatus(String token) {
        int responseStatus = Unirest.get(TEST_AREA_URL).header("Authorization", "Bearer " + token).asString()
                .getStatus();
